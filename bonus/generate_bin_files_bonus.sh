...
total_files=$(ls videos/rt_files_bonus/*.rt 2>/dev/null | wc -l)
if [ "$total_files" -eq 0 ]; then
    echo "Aucun fichier .rt trouvé dans le répertoire videos/rt_files_bonus."
    exit 1
fi
...
for ((i=0; i<total_files; i++)); do
    rt_file="videos/rt_files_bonus/file_$i.rt"
